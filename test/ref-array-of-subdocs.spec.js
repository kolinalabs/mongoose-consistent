...
const createItems = async () => {
    ...
    const itemB = await ItemB.create({
        refArrayOfObjectRelated: [{ itemA }, { itemA }, { itemA }],
    })
...
const AssertFor = {
    async restrict({ itemA, itemB, error }) {
        const message = constraintErrorMessage(
            'ItemB.refArrayOfObjectRelated.$.itemA'
        )
    ...
    async cascade({ itemA, itemB, error }) {
        ...
        expect(itemB2).not.toBeNull()
        expect(itemB2.refArrayOfObjectRelated.length).toBe(0)
    },
    async set_null({ itemA, itemB, error }) {
        ...
        expect(itemA2).toBeNull()
        expect(itemB2).not.toBeNull()
        expect(itemB2.refArrayOfObjectRelated.length).toBe(3)

        for (const objRelated of itemB2.refArrayOfObjectRelated) {
            expect(objRelated.itemA).toBeNull()
        }
    },
}
...
describe(`Mapping type ref (array of Subdocs) for action [${action}]`, () => {
